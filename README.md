...
Сервис корректно работает как при отсутствии Redis на старте, так и при его внезапном отключении во время работы.

### Как это работает

1. **Отказ Redis при старте**: если Redis недоступен при запуске сервиса, приложение не падает — `RedisManager` переходит в состояние `DISCONNECTED` и запускает фоновую задачу автоматического переподключения с экспоненциальной задержкой (от 10 до 300 секунд).

2. **Отказ Redis во время работы**:
   - **Кеширование** (`fastapi-cache`) — перестаёт работать, запросы обрабатываются напрямую из БД. Без Redis кеш просто не заполняется, функциональность сервиса не страдает.
   - **Blacklist refresh-токенов** — перестаёт блокировать старые токены. Аутентификация продолжает работать — access-токены по-прежнему проверяются по подписи и сроку действия, refresh-токены валидируются через декодирование JWT.
   - **Health-эндпоинт** (`GET /health/`) — возвращает `500 Internal Server Error` с указанием проблемы с Redis.

3. **Автоматическое восстановление**: как только Redis снова становится доступен, `RedisManager` автоматически подключается (фоновый reconnection loop), и все функции (кеширование, blacklist) восстанавливаются без перезапуска сервиса.

### Что НЕ работает при отключённом Redis

- Кеширование ответов API (запросы ходят напрямую в БД)
- Блокировка повторного использования refresh-токенов (logout и refresh не заносят токены в blacklist)

Все критичные функции (аутентификация, бронирование, управление комнатами) остаются работоспособными.
...
ПРИМЕЧАНИЕ: **fastapi-cache** использует Cache-Control браузера и устанавливает значеине max-age в значение указанное как expire кеша. То есть если пользоваться стандартной документацией FastAPI swagger по пути /docs, то браузер даже не будет отправлять запросы на backend и отдавать результаты из кеша браузера, а значит инвалидация кеша работать не будет. Для избежания таких проблем стоит отключить кеш в браузере (dev tools в разделе network кнопка disable cache) ИЛИ использовать утилиту `curl`
