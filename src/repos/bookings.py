...
class BookingRepository(BaseRepository):
    ...
    async def get_active_bookings_by_date(
        ...
        query = select(self.mapper.db_model).filter_by(
            booking_date=booking_date, status=StatusBookingEnum.ACTIVE.value
        )
        if room_id is not None:
            query = query.filter_by(room_id=room_id)
        result = await self.session.execute(query)
        return [
            self.mapper.map_to_schema(booking) for booking in result.scalars().all()
        ]
