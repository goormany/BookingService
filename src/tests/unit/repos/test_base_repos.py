...
class TestGetFiltred:
    async def test_get_filtred_without_pagination(
        self, repo: BaseRepository, mock_session: AsyncSession
    ):
    ...
    async def test_get_filtred_with_pagination(self, repo, mock_session):
        ...
        assert (
            call_args._offset is not None and call_args._offset == (page - 1) * per_page
        )


class TestGetAll:
    @pytest.mark.parametrize("page, per_page", [(None, None), (1, 5), (2, 5)])
...
class TestGetOneOrNone:
    async def test_get_one_or_none_ret_not_none(
        self, repo, mock_session, mock_model, args, kwargs
    ):
        mock_result = MagicMock()
...
class TestAdd:
    ...
    async def test_add_raises_unique_violation(self, repo, mock_session, mock_schema):
        ...
        integrity_error = IntegrityError(
            statement="stmt", params={}, orig=mock_unique_error
        )
    ...
    async def test_add_raises_foreig_key_violation(
        self, repo, mock_session, mock_schema
    ):
        mock_error = MagicMock()
        mock_error.__cause__ = MagicMock(spec=ForeignKeyViolationError)

        integrity_error = IntegrityError(statement="stmt", params={}, orig=mock_error)
