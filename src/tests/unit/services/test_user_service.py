...
@pytest.fixture
def user_in():
    return UserIn(username="test", password="test_password")
...
class TestCreateUser:
    async def test_success(self, service, user_in, mock_hashed_password):
        with patch.object(
            AuthServices, "get_password_hash", return_value=mock_hashed_password
        ):
...
class TestGetUserWithPassword:
    async def test_success(self, service, args, kwargs):
        service.db.users.get_user_with_hashed_password.return_value = (
        ...
        service.db.users.get_user_with_hashed_password.assert_called_once_with(
            *args, **kwargs
        )

    async def test_not_found_user(self, service, args, kwargs):
        service.db.users.get_user_with_hashed_password.side_effect = (
            UserNotFoundException
        )
...
class TestChangeUserRole:
    ...
    async def test_success(self, service, role):
        ...
        result = await service.change_user_role(
            user_id=user_id, role=UserRoleSchema(role=role)
        )
