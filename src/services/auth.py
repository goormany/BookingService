...
from src.utils.exceptions.exceptions import ExpiredJWTTokenException, InvalidTokenDecodedException
...
class AuthServices(BaseServices):
    ...
    def decode_access_token(token: str) -> JWTData:
        try:
            jwt_token = jwt.decode(token, settings.JWT_SECRET_KEY, [settings.JWT_ALGORITHM])
