...
class BookingService(BaseServices):
    ...
    async def get_all_bookings_adm(
        self, per_page: int, page: int, *args, **kwargs
        ...
        return await self.db.bookings.get_filtred(
            per_page=per_page, page=page, *args, **kwargs
        )

    async def create_booking(
        self, booking_data: BookingIn, room_id: int, user_id: int
    ) -> BookingResponse:
        try:
            new_booking_data = BookingCreate(
                **booking_data.model_dump(), room_id=room_id, user_id=user_id
            )
    ...
    async def get_my_bookings(
        self, per_page: int, page: int, user_id: int
    ) -> list[BookingResponse]:
        return await self.db.bookings.get_filtred(
    ...
    async def get_bookings_by_room(
        ...
        **kwargs,
        ...
        return await self.db.bookings.get_filtred(
            per_page=per_page, page=page, room_id=room_id, *args, **kwargs
        )
    ...
    async def soft_delete_booking(self, *args, **kwargs) -> BookingResponse:
        try:
            ...
            booking = await self.db.bookings.edit(
                soft_delete_data, True, *args, **kwargs
            )
        except BookingRoomsNotFoundObjException:
            raise BookingNotFoundException
        await self.db.commit()
